...
where dich_vu.id_dich_vu in (select hop_dong.id_dich_vu from hop_dong where (year(ngay_lam_hop_dong) = 2018))
and dich_vu.id_dich_vu not in (select hop_dong.id_dich_vu from hop_dong where (year(ngay_lam_hop_dong) = 2019));